...
/**
 * Implements a dialog for editing a Language object. Contains two text fields,
 * one for the id, one for the description. The ID field is validated against a
 * list of existing Language IDs which are not allowed as values. The
 * description field will produce a warning when it's empty.
 * dataRetrievalListeners will obtain a language object when OK is clicked,
 * where isOnlyUsed is set to false and supported is set to 0. <br>
 * This dialog disappears, when losing focus or when ESCAPE is pressed. OK is
 * set to be the default button. Initial focus is set to the description field,
 * if it is empty and the id field is not, and to the id field otherwise
 * 
 * @author Simon Hampe
 * 
 */
public class LanguageEditor extends FrameworkDialog implements
		ResourceDependent {
	...
	private Action disposalListener = new AbstractAction() {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == okButton)
				fireDataReady(new Language(idText.getText(),
						descText.getText(), false, 0));
			dispose();
		}
	};
	 ...
	 * 
	 * @param tabooList
	 *            A list of language ids that are not allowed. If you want to
	 *            edit an existing language, its ID should not be contained in
	 *            the list.
	 * @param language
	 *            If null, all fields are initialized as empty. If not null, id
	 *            and description are copied from this language
	 */
	public LanguageEditor(Collection<String> tabooList, Language language,
			ResourceReference r, PolyglotStringLoader l, String lid) {
		super(r, l, lid);
		// Copy data
		if (tabooList != null)
			this.tabooList.addAll(tabooList);
		oldLanguage = language;
		
		// Load ImageIcon
		warnIcon = new ImageIcon(resource.getFullResourcePath(this,
				"graphics/LanguageEditor/warn.png"));
		...
		descLabel
				.setText(loader.getString(sgroup + ".description", languageID));
		...
		if (language != null && (language.description.equals("")))
			descText.requestFocusInWindow();
		else
		...
		getRootPane().getInputMap(JComponent.WHEN_IN_FOCUSED_WINDOW).put(
				KeyStroke.getKeyStroke("ESCAPE"), "dispose");
		getRootPane().getActionMap().put("dispose", disposalListener);

		// Initialize validators to set correct tooltips and link them to
		// components
			...
			public Result validate(JTextComponent component) {
				...
				if (result == Result.INCORRECT) {
					if (component.getText().trim().equals(""))
						setToolTipText(component, loader.getString(sgroup
								+ ".idempty", languageID));
					else
						setToolTipText(component, loader.getString(sgroup
								+ ".idnotunique", languageID));
				}
				return result;
			}
		};
	...
	/**
	 * Returns the edit generated by the current input (which is not necessarily a valid one) or null,
	 * if f == null
	 */
	public UndoableLanguageEdit getUndoableLanguageEdit(UndoableEditFactory f) {
		return f == null? null : f.createUndoableLanguageEdit(oldLanguage, new Language(idText.getText(),descText.getText(),false,0));
	}
	
	/**
	 * Tries to perform the edit represented by the current input of this editor. This should only be called from a 
	 * dataRetrievalListener, after dataReady has been called, since only this guarantees validity.
	 * @throws CannotRedoException - if the operation cannot be performed or f == null
	 */
	public UndoableLanguageEdit performUndoableLanguageEdit(UndoableEditFactory f) throws CannotRedoException {
		if(f == null) throw new CannotRedoException(); 
		else return  f.performUndoableLanguageEdit(oldLanguage, new Language(idText.getText(), descText.getText(), false, 0));
	}
	...
	/**
	 * Assigns a resource reference. If r == null, the default reference is
	 * used.
	 */
	@Override
