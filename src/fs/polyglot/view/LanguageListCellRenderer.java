...
public class LanguageListCellRenderer implements ListCellRenderer,
	...
	/**
	 * Constructs a cell renderer using the specified resource reference and
	 * languageID. Null values indicate that the fsframework default values
	 * should be used
	 */
	public LanguageListCellRenderer(ResourceReference r,
	...
	public Component getListCellRendererComponent(final JList list,
		...
		if (value instanceof Language) {
			...
			String color = lang.isFullySupported() ? fullySupportedHTML
					: notfullySupportedHTML;
			String desc = lang.isOnlyUsed ? "<i>"
					+ loader.getString(sgroup + ".nodescription", languageID)
					+ "</i>" : lang.description;
			sumPanel
					.setToolTipText((lang.isOnlyUsed
							|| !lang.isFullySupported() ? ("<html>"
							+ (lang.isOnlyUsed ? "- "
									+ loader.getString(sgroup + ".onlyused",
											languageID) : "")
							+ (lang.isOnlyUsed && !lang.isFullySupported() ? "<br>"
									: "")
							+ (!lang.isFullySupported() ? "- "
									+ loader.getString(sgroup
											+ ".notfullysupported", languageID)
									: "") + "</html>")
							: null));
			String text = "<html><font color=" + color + ">" + lang.id
					+ "</font>: " + desc + "</html>";
			label.setText(text);

			bar.setStringPainted(true);
			bar.setValue(lang.supported);
			// bar.setToolTipText(loader.getString(sgroup + ".bartooltip",
			// languageID));
		}
	...
	/**
	 * Assigns a resource reference (if null, the FsfwDefaultReference is used)
	 * and reloads the icons. This doesn't cause any repaints
	 */
	@Override
	public void assignReference(ResourceReference r) {
		reference = (r != null) ? r : FsfwDefaultReference
				.getDefaultReference();
		okIcon = new ImageIcon(reference.getFullResourcePath(this,
				"graphics/LanguageListCellRenderer/ok.png"));
		undescribedIcon = new ImageIcon(reference.getFullResourcePath(this,
				"graphics/LanguageListCellRenderer/undescribed.png"));
		warnIcon = new ImageIcon(reference.getFullResourcePath(this,
				"graphics/LanguageListCellRenderer/warn.png"));
	}
