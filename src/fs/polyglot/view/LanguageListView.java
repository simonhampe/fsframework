...
/**
 * This class implements a panel containing a list of PolyglotStringTable languages as rendered by LanguageListCellRenderer, together with
 * some buttons to manipulate it
 * @author Simon Hampe
 *
 */
public class LanguageListView extends JPanel implements ResourceDependent {
		...
		public void mouseClicked(MouseEvent e) {
			...
			if(e.getClickCount() >= 2) {
				if(languageList.getSelectedIndex() != -1) {
					editorListener.actionPerformed(new ActionEvent(editButton,0,null));
				}
			}
		}
	};
	...
	/**
	 * Toggles the activation state of the buttons depending on the current selection
	 */
	private ListSelectionListener selectionListener = new ListSelectionListener()  {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			if(languageList.getSelectedValue() == null) {
				...
				///For deleteButton also check, if the selected language is an undescribed language
	...
	private ActionListener deleteListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			editFactory.performUndoableLanguageEdit((Language)languageList.getSelectedValue(), null);
		}
	};
	...
	private ActionListener editorListener = new ActionListener() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(e.getSource() == editButton) {
				editor = createEditor((Language)languageList.getSelectedValue());
			}
			if(editor != null) {
	...
	private DataRetrievalListener editListener = new DataRetrievalListener() {
		@Override
		public void dataReady(Object data) {
			editFactory.performUndoableLanguageEdit((Language)languageList.getSelectedValue(), (Language)data);
		}
	};
	...
	/**
	 * Constructs a language list view, containing a scrollable list view of the language list of a polyglot string table together with buttons for
	 * operations. This list view will automatically link itself to the TableUndoManager associated to the table
	 * @param r The resource reference for this component and created dialogs. If null, default reference is used
	 * @param stringloader The string loader for the tooltip strings and all created dialogs. If null, the default loader is used.
	 * @param languageID The language id for this component and created dialogs. If null, the global language id is used
	 * @param associatedTable The associated PolyglotTableModel, the data of which is displayed. If null, an empty table is created.
	 */
	public LanguageListView(ResourceReference r, PolyglotStringLoader stringloader, String languageID, PolyglotTableModel associatedTable)  {
		super();
		//Copy data
		assignReference(r);
		...
		this.editFactory = new UndoableEditFactory(this.table, this.loader, this.languageID, TableUndoManager.getUndoManager(associatedTable));
		...
		languageList.getSelectionModel().addListSelectionListener(selectionListener);
		...
		newButton.setToolTipText(loader.getString(sgroup + ".newtooltip", this.languageID));
		buttonbar.add(newButton);
		deleteButton.setIcon(deleteIcon);
		deleteButton.setToolTipText(loader.getString(sgroup + ".deletetooltip", this.languageID));
		buttonbar.add(deleteButton);
		editButton.setIcon(editIcon);
		editButton.setToolTipText(loader.getString(sgroup + ".edittooltip", this.languageID));
	...
	/**
	 * Creates a language editor for editing the specified language. If l == null, this creates an
	 * empty editor and registers a listener for adding a language upon confirmation. Otherwise it registers a
	 * listener for changing the currently selected language to the resulting language. The editor
	 * is placed outside this list view just beside the currently selected language (if any is selected) or at the top of the list.
	 */
	protected LanguageEditor createEditor(Language l) {
		...
		if(l != null) tabooList.remove(l.id);
		//Create dialog
		LanguageEditor editor = new LanguageEditor(tabooList,l, reference, loader, languageID);
		//If l is null, this is a language addition, but also if l!= null but not contained in the official table language list
		//(in this case, an only-used language is added)
		editor.addDataRetrievalListener(l == null? newListener : table.containsLanguage(l.id)? editListener : newListener);
		//Set Bounds
		int index = languageList.getSelectedIndex();
		int y = 0;
		if(index != -1)  y = languageList.getCellBounds(index, index).y;
		int realx = languageList.getLocationOnScreen().x;
		int realy = languageList.getLocationOnScreen().y;
		editor.setBounds(realx-editor.getWidth(),realy+y,editor.getWidth(), editor.getHeight());
		//Return
		return editor;
	}
	...
	/**
	 * Sets the resource reference and reloads the icons. If r == null, the default reference is used.
	 */
	@Override
	public void assignReference(ResourceReference r) {
		...
		newIcon = new ImageIcon(reference.getFullResourcePath(this, path + "new.png"));
		deleteIcon = new ImageIcon(reference.getFullResourcePath(this, path + "delete.png"));
		editIcon = new ImageIcon(reference.getFullResourcePath(this, path + "edit.png"));
	}
