...
public class TabooValidator extends LabelIndicValidator<JTextComponent> {
	...
	/**
	 * Constructs a {@link LabelIndicValidator}, which regards a JTextField's
	 * content as correct, if it is not empty and not in the tabooList, and
	 * otherwise as incorrect.
	 * 
	 * @param correct
	 *            The Icon for the associated label, if the content is correct
	 * @param warning
	 *            never used
	 * @param incorrect
	 *            The icon for the associated label, if the content is incorrect
	 * @param tabooList
	 *            A list of strings which are not valid content
	 * @param Indicates
	 *            whether an empty text field should be regared as correct or
	 *            not
	 */
	public TabooValidator(ImageIcon correct, ImageIcon warning,
			ImageIcon incorrect, Collection<String> tabooList,
			boolean canBeEmpty) {
	...
	/**
	 * This sets the list of incorrect values and notifies all change listeners.
	 * If it is null, it is interpreted as the empty list.
	 */
	...
	/**
	 * Sets whether the empty string is allowed as content and notifies all
	 * change listeners if the new value is different.
	 */
	...
	/**
	 * Returns CORRECT, if the TextFields content is not in the taboo list
	 * (after trimming of whitespaces) and, if canBeEmpty has been set to false,
	 * if it isn't empty (after trimming). Otherwise returns INCORRECT. Returns
	 * INCORRECT for a null component
	 */
	@Override
	public Result validate(JTextComponent component) {
		...
		// Empty text?
		if (!canBeEmpty && component.getText().trim().equals(""))
			return Result.INCORRECT;
		// TabooList
		for (String taboo : tabooList) {
			if (component.getText().trim().equals(taboo.trim()))
				return Result.INCORRECT;
		}
