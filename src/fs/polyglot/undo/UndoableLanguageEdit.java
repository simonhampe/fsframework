 ...
 * polyglot string table. Usually the user should not create an instance of this
 * class directly, but via an UndoableEditFactory instance.
 * 
 * @author Simon Hampe
 * 
 */
public class UndoableLanguageEdit extends AbstractUndoableEdit {
	 ...
	 *            considered without effect and it will not post itself
	 *            automatically
	 * @param table
	 *            The table in which the change was performed. If this is null,
	 *            the edit has no effect.
	 * @param loader
	 *            The string loader for representation names. If null, the
	 *            default loader is used. If UndoableEdits are created via an
	 *            UndoableEditFactory, the factory already has obtained the
	 *            string loader and passes it on.
	 * @param languageID
	 *            The language in which to create the representation string. If
	 *            null, the global language id is used. If UndoableEdits are
	 *            created via an UndoableEditFactory the factory uses a common
	 *            language ID for all created edits.
	 * @throws NullPointerException
	 *             - if table == null
	 */
	public UndoableLanguageEdit(Language oldValue, Language newValue,
	 ...
	 *         - This edit represents a language change and the new language id
	 *         exists in the table <br>
	 *         - This edit represents a language addition and the new language
	 *         id still exists <br>
	 *         - This edit represents a language removal and the old language id
	...
	/**
	 * @return true, if and only if one of the following statements is true: <br>
	 *         - This edit represents a language change and the old language id
	 *         exists in the table <br>
	 *         - This edit represents a language addition and the new language
	 *         id does not exist in the table <br>
	 *         - This edit represents a language removal and the old language id
	 *         exists in the table <br>
	 *         - Both old and new value are null
	 */
	 ...
	 */
	public String getLanguageEditPresentationName(Language oldval,
			Language newval) {
		// Null edit
		if (oldval == null && newval == null)
			return loader.getString("fs.polyglot.undo.languagegeneral",
					languageID, "null", "null");
		// Addition
		if (oldval == null)
			return loader.getString("fs.polyglot.undo.languageadd", languageID,
					newval.description);
		// Removal
		if (newval == null)
			return loader.getString("fs.polyglot.undo.languageremove",
					languageID, oldval.description);
		// Change in description
		if (oldValue.id.equals(newValue.id)
				&& !oldValue.description.equals(newValue.description))
			return loader.getString("fs.polyglot.undo.languagedescription",
					languageID, oldval.description, newval.description);
		// Other changes
		return loader.getString("fs.polyglot.undo.languagegeneral", languageID,
	...
	/**
	 * Convenience method which changes a language oldval to newval in the
	 * language list of the specified table
	 * 
	 * @param table
	 *            The table in which the change should be effected (If null,
	 *            then this call has no effect)
	 * @param oldval
	 *            The old value. If null, this is considered an addition of a
	 *            language
	 * @param newval
	 *            The new value. If null, this is considered a removal of a
	 *            language (if both are null, this call has no effect)
	 * @throws UnsupportedOperationException
	 *             - If this operation cannot be performed
	 */
	public static void performLanguageEdit(PolyglotTableModel table,
			Language oldval, Language newval)
			throws UnsupportedOperationException {
		if (table == null || (oldval == null && newval == null))
			return;
		// Addition
		if (oldval == null) {
			if (table.containsLanguage(newval.id))
		...
		// Removal
		if (newval == null) {
			if (!table.containsLanguage(oldval.id))
				throw new UnsupportedOperationException();
