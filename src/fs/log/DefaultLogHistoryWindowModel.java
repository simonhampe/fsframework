...
/**
 * Provides the standard model implementation for LogHistoryWindow.
 * Appended messages are stored internally in a sorted list as a 
 * string conforming to the appendes layout together with the log events
 * level. This class also supports the basic level-oriented filtering mechanism defined by
 * AbstractLogHistory: For each Level 
 * a boolean value can be configured that specifies if messages of this level should 
 * be returned by getLogHistory(). By default, all predefined levels from INFO upwards are displayed. 
 * Since this class is derived from AppenderSkeleton, threshold filtering 
 * and general filters are supported.
 * 
 * @author Simon Hampe
 *
 */
public class DefaultLogHistoryWindowModel extends AbstractLogHistoryModel {
	 ...
	 * getLogHistory. If a message has a level not included in the key set, it will be displayed.
	 */
	...
	public DefaultLogHistoryWindowModel() {
		...
		for(Level l : Arrays.asList(Level.INFO, Level.WARN, Level.ERROR, Level.FATAL)) {
	...
	/**
	 * Controls whether messages of the specified level 
	 * should be displayed. If a value has already been specified,
	...
	/**
	 * Returns whether messages of a certain level are displayed. If no 
	 * value has been specified (e.g. if l is a user-defined level that has
	 *  not yet been configured), true is returned, since by default, all 
	 *  unconfigured levels are displayed.
	 */
	...
	private class DefaultHTMLLayout extends Layout {
		 ...
		 */
		private final PatternLayout internalPattern = new PatternLayout(DLHWM_PATTERN_LAYOUT);
		
		/**
		 * Formats the event to an html < pre > - string containing the log message.
		...
		public String format(LoggingEvent arg0) {
			...
			if(arg0.getLevel().isGreaterOrEqual(Level.INFO) && !Level.INFO.isGreaterOrEqual(arg0.getLevel())) {
				//If > WARN (that is: >= and WARN is not <=)
				if(arg0.getLevel().isGreaterOrEqual(Level.WARN) && !Level.WARN.isGreaterOrEqual(arg0.getLevel())) {
					colorcode = "#FF0000"; //RED
				}
				else colorcode = "#FFCC11"; // ORANGE
			}
			return String.format("<pre> <font color=\'%s\'>%s</font></pre>", colorcode, internalPattern.format(arg0));
		}
