...
public class StringTableTest {
	...
	public static void main(String[] args) {
		try {
			...
			Document in = XMLToolbox.loadXMLFile(new File("examples/PolyglotStringTable.xml"));
			PolyglotStringTable ex = 
				new PolyglotStringTable(in,
			...
			Document up = XMLToolbox.loadXMLFile(new File("examples/PolyglotStringTableUpdate.xml"));
			...
			showfullReport(ex);
			HashMap<String,HashSet<String>> ms =
				ex.getMissingStrings();
	...
	public static void showfullReport(PolyglotStringTable ex) {
		...
		for(String lid : ex.getUsedLanguages()) {
			System.out.println(" - "  + ex.getLanguageDescription(lid) + " (" + lid + ")"
					+ ", isOnlyUsed: " + !ex.getLanguageList().contains(lid) 
					+ ", supported: " + ex.getSupport(lid));
		}
		...
		for(String sid : ex.getIDList()) {
			System.out.println("- " + sid + " (group: " +
					ex.getGroupID(sid) + ")");
			for(String lid : ex.getLanguageList()) {
				String s = ex.getUnformattedString(sid, lid);
				if(s == null) s = "[missing]";
				System.out.println("- - " + ex.getLanguageDescription(lid)+ ": " + s );
			}
		}
	}
}
